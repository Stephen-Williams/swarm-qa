...
testpath = "."
testfile = "pid"
filename = "test_results_get-status-" + testpath.replace("/", "-") + "_" + time.strftime("%Y-%m-%d_%H-%M-%S") + ".csv"
...
def get(ncrid, expected):

    arg = { 'NECTAR_CRID' : ncrid }

    try:
        answer = json.loads(web_call(r'http://{}:{}/swarm/campaign/resume'.format(server, port), arg, expected))
